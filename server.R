Server <- function(input, output, session) {
  ...
  showNotification("Pemetaan TwiSAR", duration = NULL, type = "message")

  output$currentTime <- renderText({
    invalidateLater(1000, session)
    paste("Waktu saat ini : ", Sys.time())
  })
...
#/////////////////////////////////////////////////////////////////////////////////////////////////////////////////Crawling
  ...
  withProgress(message = 'Download Tweet Banjir', value = 0, {
...
#Banjir
  ...
  output$link1 <- renderUI({
    linkurl1 <- m[1,3]
    tags$a(href=linkurl1, class="btn btn-info","Lihat postingan")
  })
...
#Gelombng
  ...
  output$link8 <- renderUI({
    linkurl2 <- L[2,3]
    tags$a(href=linkurl2, class="btn btn-info","Lihat postingan")
  })
...
#    addProviderTiles(providers$Esri.WorldTopoMap, group = "World Topography") %>%
  ...
  #  addProviderTiles("OpenStreetMap.Mapnik") %>%
    ...
    addLayersControl(
      overlayGroups =  c("Banjir", "Angin Kencang", "Gelombang Tinggi", "Tanah Longsor", "Gempa Bumi", "Railway", "Region"),
